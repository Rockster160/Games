...
class Algorithm {
  ...
  static genCell(dungeon, cell) {
    cell = cell || sample(dungeon.cells) || dungeon.findOrGenCell()
    let dir = sample(Object.keys(Algorithm.DIRS))
    let [x, y] = Algorithm.DIRS[dir]

    if (dungeon.branches == "low") {
      // The more branches exist, the less likely it is to add a new one
      if (rand(cell.neighbors() + 1) > 0) { return }
